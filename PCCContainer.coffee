...
class PCCConstantContainer extends PCCContainer
	...
	isReadonly: -> true
	ccsTree: -> new CCS.ConstantExpression(@value)
...
class PCCVariableContainer extends PCCContainer
	...
	isReadonly: -> false
	ccsTree: -> new CCS.VariableExpression(@identifier)
	...
	constructor: (@leftContainer, @rightContainer, @operator) ->
		if @operator == "+" or @operator == "^"
			if @leftContainer.ccsType.isString() or @rightContainer.ccsType.isString() or @operator == "^"
				@exp = new CCS.ConcatenatingExpression(@leftContainer.ccsTree(), @rightContainer.ccsTree())
				super PCCType.STRING
			else
				@exp = new CCS.AdditiveExpression(@leftContainer.ccsTree(), @rightContainer.ccsTree(), @operator)
				super PCCType.INT
		else if @operator == "-"
			@exp = new CCS.AdditiveExpression(@leftContainer.ccsTree(), @rightContainer.ccsTree(), @operator)
			super PCCType.INT
		else if @operator == "*" or @operator == "/" or @operator == "%"
			@exp = new CCS.MultiplicativeExpression(@leftContainer.ccsTree(), @rightContainer.ccsTree(), @operator)
			super PCCType.INT
		else if @operator == "<" or @operator == "<=" or @operator == ">" or @operator == ">="
			@exp = new CCS.RelationalExpression(@leftContainer.ccsTree(), @rightContainer.ccsTree(), @operator)
			super PCCType.BOOL
...
class PCCUnaryContainer extends PCCComposedContainer
	constructor: (@operator, @container) ->
		if @operator == "!"
			...
			@exp = new CCS.ComplementExpression(@container.ccsTree())
			super PCCType.BOOL
		else if @operator == "-"
