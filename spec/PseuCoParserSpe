...
programs = 
	...
	"Listing 6.2":
		...
		mainAgent {
			a1 = start countDutch (); // Ich lass einen Agenten los !
			a2 = start countFrench (); // Und noch einen !
	...
	"Listing 6.3":
		...
		mainAgent {
			a1 = start factorial (3);
			a2 = start factorial (5);
		}
	...
	"Listing 7.2":
		...
		mainAgent {
			a1 = start factorial ( cc );
			// Ich erzeugen einen Agenten .
			a2 = start factorial ( cc );
			// Und der andere auch .
			dd <! 3;
	...
	"Listing 7.5":
		...
		mainAgent {
			int j ;
			for ( j =0; j <4 ; j ++){
				a [ j ] = start factorial ( ff , dd , cc );
				// Ich erzeuge ein paar Agenten .
			}
			for ( j =1; j <=8 ; j ++){
				dd <! j ;
				// Hier gibt ’s Arbeit .
